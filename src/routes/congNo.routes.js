...
router.get("/doi-tac", authenticate, async (req, res, next) => {
  try {
    const filters = {
...
router.get("/doi-tac/chi-tiet", authenticate, async (req, res, next) => {
  try {
    ...
    const data = await congNoService.getChiTietDoiTac(ma_doi_tac, loai_cong_no);
