...
class HoaDonBanService {
  ...
  async taoHoaDon(data) {
    const { so_hd, ngay_ban, ma_kho_xuat, ma_kh, nguoi_tao, ghi_chu } = data;
    ...
    const result = await pool.query(
      ...
      RETURNING *`,
      [so_hd, ngay_ban, ma_kho_xuat, ma_kh, nguoi_tao, TRANG_THAI.NHAP, ghi_chu]
    );
  ...
  async themXe(so_hd, xe_key, don_gia) {
    ...
    try {
      ...
      if (hdResult.rows[0].trang_thai !== TRANG_THAI.NHAP) {
        throw new Error(`Không thể sửa hóa đơn ở trạng thái ${hdResult.rows[0].trang_thai}`);
      }
  ...
  async pheDuyet(so_hd, nguoi_duyet) {
    ...
    try {
      ...
      if (hd.trang_thai !== TRANG_THAI.GUI_DUYET) {
        throw new Error(`Không thể duyệt hóa đơn ở trạng thái ${hd.trang_thai}`);
      }
      ...
      for (const ct of chiTietResult.rows) {
        if (ct.loai_hang === 'XE') {
          ...
          await client.query(
            ...
            [
              ct.xe_key, 'BAN_HANG', so_hd, hd.ma_kho_xuat,
              ct.don_gia, nguoi_duyet, `Bán xe theo hóa đơn ${so_hd}`
            ]
          );
          ...
          await client.query(
            ...
            [
              ct.ma_pt, 'BAN_HANG', so_hd, hd.ma_kho_xuat,
              -ct.so_luong, ct.don_gia, -ct.thanh_tien,
              nguoi_duyet, `Bán hàng theo hóa đơn ${so_hd}`
            ]
          );
      ...
      await client.query(
        ...
        [
          'PT-' + so_hd, hd.ngay_ban, 'THU', hd.ma_kho_xuat, hd.ma_kh,
          hd.thanh_toan, TRANG_THAI.DA_DUYET, nguoi_duyet, so_hd,
