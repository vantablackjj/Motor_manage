...
class PhuTungKhoaService {
   ...
   * KHÓA PHỤ TÙNG
   */
  static async lock({ ma_pt, ma_kho, so_phieu, loai_phieu, so_luong, ly_do }) {
    ...
    try {
      ...
      await client.query(
        ...
        (ma_hang_hoa, ma_kho, so_phieu, loai_phieu, so_luong_khoa, ngay_khoa, ly_do)
    ...
    try {
      ...
      for (const row of khoaRes.rows) {
        await client.query(
          ...
          WHERE ma_hang_hoa = $2 AND ma_kho = $3
          `,
          [row.so_luong_khoa, row.ma_hang_hoa, row.ma_kho],
        );
      }
  ...
  static async getByKho(ma_kho) {
    const result = await pool.query(
      ...
      SELECT
        ...
        k.ngay_khoa,
        k.ly_do
      FROM tm_hang_hoa_khoa k
      JOIN tm_hang_hoa pt ON pt.ma_hang_hoa = k.ma_hang_hoa
      WHERE k.ma_kho = $1
