...
class BaoCaoService {
  ...
  async tonKhoXe(filters = {}) {
    ...
    let sql = `
      ...
      AND (pt.ma_nhom_hang IN (SELECT group_code FROM fn_get_all_child_groups('XE'::text)) OR pt.ma_nhom_hang = 'XE')
    `;
    const params = [];
    if (ma_kho) {
      params.push(ma_kho);
  ...
  async doanhThuTheoSanPham(filters = {}) {
    ...
    if (loai === "XE") {
      ...
      sql = `
        ...
        WHERE h.trang_thai IN ('DA_THANH_TOAN', 'DA_GIAO') 
        AND (pt.ma_nhom_hang NOT IN (SELECT group_code FROM fn_get_all_child_groups('XE'::text)) OR pt.ma_nhom_hang IS NULL)
      `;
    }
  ...
  async nhapXuatXe(filters = {}) {
    ...
    let sql = `
      ...
      WHERE (
        pt.ma_nhom_hang IN (SELECT group_code FROM fn_get_all_child_groups('XE'::text)) 
  ...
  async nhapXuatPhuTung(filters = {}) {
    ...
    let sql = `
      ...
      WHERE (
        pt.ma_nhom_hang NOT IN (SELECT group_code FROM fn_get_all_child_groups('XE'::text)) 
