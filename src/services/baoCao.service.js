...
class BaoCaoService {
  ...
  async tonKhoXe(filters = {}) {
    ...
    let sql = `
      ...
      AND (pt.ma_nhom_hang IN (SELECT x FROM get_nhom_hang_children_v2('XE') as x) OR pt.ma_nhom_hang = 'XE')
    `;
    const params = [];
    if (ma_kho) {
      params.push(ma_kho);
  ...
  async tonKhoPhuTung(filters = {}) {
    ...
    let sql = `
      ...
      WHERE (pt.ma_nhom_hang NOT IN (SELECT x FROM get_nhom_hang_children_v2('XE') as x) OR pt.ma_nhom_hang IS NULL)
    `;
    const params = [];
    if (ma_kho) {
      params.push(ma_kho);
  ...
  async doanhThuTheoSanPham(filters = {}) {
    ...
    if (loai === "XE") {
      ...
      sql = `
        ...
        WHERE h.trang_thai IN ('DA_THANH_TOAN', 'DA_GIAO') 
        AND (pt.ma_nhom_hang NOT IN (SELECT x FROM get_nhom_hang_children_v2('XE') as x) OR pt.ma_nhom_hang IS NULL)
      `;
    }
  ...
  async nhapXuatXe(filters = {}) {
    ...
    let sql = `
      ...
      WHERE (
        pt.ma_nhom_hang IN (SELECT x FROM get_nhom_hang_children_v2('XE') as x) 
  ...
  async nhapXuatPhuTung(filters = {}) {
    ...
    let sql = `
      ...
      WHERE (
        pt.ma_nhom_hang NOT IN (SELECT x FROM get_nhom_hang_children_v2('XE') as x) 
